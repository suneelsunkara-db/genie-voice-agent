...
class MultilingualASRModel(mlflow.pyfunc.PythonModel):
    ...
    def load_context(self, context: mlflow.pyfunc.PythonModelContext) -> None:
        ...
        if self.family == "funasr":
            ...
            vad_dir = (context.artifacts or {}).get("vad_dir")
            if vad_dir:
                kwargs["vad_model"] = vad_dir
            elif self.metadata.get("funasr_vad_model"):
                kwargs["vad_model"] = self.metadata["funasr_vad_model"]
                hub = self.metadata.get("funasr_hub")
                if hub:
                    kwargs["hub"] = hub
    ...
    def _transcribe(self, audio_path: str, row: dict[str, Any]) -> str:
        ...
        if self.family == "funasr":
            ...
            variant = str(self.metadata.get("funasr_variant") or "")
