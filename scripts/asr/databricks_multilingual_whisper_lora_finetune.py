"""Fine-tune a multilingual Whisper-family ASR model with LoRA on Databricks."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fine-tune multilingual Whisper with LoRA.")
    ...
    if not train_rows:
        raise SystemExit(f"No train rows found for language={args.language_code} split={args.train_split}")
    if not eval_rows:
        raise SystemExit(
            f"No validation rows found for language={args.language_code} split={args.validation_split}"
        )
    ...
    run_config = {
        ...
        "adaptation_type": "finetuned_lora",
        "fine_tuned_by_us": True,
    ...
    if args.dry_run:
        ...
        print(json.dumps({"status": "dry_run_ok", **run_config}, indent=2, ensure_ascii=False))
        return
    ...
    processor = WhisperProcessor.from_pretrained(
        args.base_model,
        language=args.language_name.lower(),
        task="transcribe",
    )
    ...
    train_dataset = train_dataset.map(
        lambda item: prepare_item(item, processor),
    ...
    print(json.dumps({"status": "train_ok", "metrics": metrics, **run_config}, indent=2, ensure_ascii=False))
    ...
    *,
    ...
    with Path(path).open("r", encoding="utf-8") as f:
        for line_no, line in enumerate(f, start=1):
            ...
            if row.get("language") != language_code or row.get("split") != split:
                continue
            rows.append(row)
