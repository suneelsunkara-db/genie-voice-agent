...
#
# Register production multilingual OSS-baseline ASR candidates in Unity Catalog:
#   1. Download/cache model snapshot once during registration.
#   2. Register candidate as an MLflow pyfunc model in UC.
#
# Fair quality evaluation belongs in 07_deep_voice_model_eval.sh against serving
# endpoints. Do not use CPU serverless pyfunc scoring for promotion decisions.
# =============================================================================
set -euo pipefail
...
usage() {
  cat <<EOF
Register production multilingual OSS-baseline ASR candidates in UC/MLflow.

Commands:
  list             Print supported candidate ids (app multilingual routes only).
...
Environment:
  ...
  ASR_ML_REGISTER_MANIFEST        Optional manifest path override for bootstrap-public-manifest.
...
Examples:
  ...
  scripts/asr/10_register_multilingual_asr_candidates.sh register-all
EOF
}
