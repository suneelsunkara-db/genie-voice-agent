...
def main() -> None:
    ...
    parser.add_argument("--family", choices=["qwen3", "whisper"], required=True)
    ...
    if args.force_download or not any(snapshot_dir.iterdir() if snapshot_dir.exists() else []):
        snapshot_download(
    ...
    with mlflow.start_run(experiment_id=experiment_id, run_name=f"register-{args.candidate_id}") as run:
        ...
        model_info = mlflow.pyfunc.log_model(
            ...
            artifacts={"model_dir": str(snapshot_dir)},
